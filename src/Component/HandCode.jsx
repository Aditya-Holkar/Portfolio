...
const handProjects = projectsData.filter(p => p.category === 'hand')

export const HandCode = memo(() => {
  return (
    <div className='flex flex-col gap-12'>
      <div className='flex flex-row gap-4 max-md:flex-col'>
        ...
        <div className='flex flex-[5] flex-col gap-2'>
          <h1 className='text-4xl font-bold tracking-tight'>HandCode</h1>
          <p className='max-w-2xl text-sm leading-relaxed' style={{ color: 'var(--text-muted)' }}>
            Hand-crafted front-end projects — built from scratch with clean code, thoughtful design, and no shortcuts.
          </p>
        </div>
      </div>

      <div className='flex flex-col gap-15'>
        {handProjects.map((params, idx) => (
          <div
            key={idx}
